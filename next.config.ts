...
const inputStyle: React.CSSProperties = {
  ...
  border: '1px solid rgba(201,168,76,0.2)', color: '#F5EDD6',
  fontFamily: "'Cormorant Garamond',serif", fontSize: '1rem',
  padding: '14px 16px', outline: 'none', letterSpacing: '0.04em',
...
const labelStyle: React.CSSProperties = {
  ...
  color: '#C9A84C', opacity: 0.8, display: 'block', marginBottom: 8,
}
...
function BookingUI() {
  ...
  if (success) {
    return (
      <div style={{ textAlign: 'center', padding: '100px 20px', maxWidth: 600, margin: '0 auto' }}>
        ...
        <h2 className="gold-text" style={{ fontFamily: "'Cinzel',serif", fontSize: '2rem', letterSpacing: '0.08em', marginBottom: 16 }}>Booking Confirmed</h2>
        <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '1.15rem', color: '#D4C4A0', lineHeight: 1.75, marginBottom: 36 }}>
          Thank you. Your session has been reserved and a confirmation has been sent to your email. Star and her team look forward to welcoming you.
        </p>
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '60px 20px 100px', display: 'grid', gridTemplateColumns: '1fr 380px', gap: 60, alignItems: 'start' }}>
      ...
      <div style={{ gridColumn: '1 / -1', textAlign: 'center', marginBottom: 20 }}>
        ...
        <h1 className="gold-text" style={{ fontFamily: "'Cinzel',serif", fontSize: 'clamp(2rem,4vw,3rem)', fontWeight: 600, letterSpacing: '0.08em', marginBottom: 12 }}>Book a Session</h1>
        <p style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: '1.1rem', color: '#D4C4A0' }}>Choose your service, select your time, and begin your transformation.</p>
      </div>
      ...
      <div>
        ...
        <div style={{ marginBottom: 48 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid rgba(201,168,76,0.12)' }}>
            <span style={{ fontFamily: "'Cinzel',serif", fontSize: '0.6rem', letterSpacing: '0.3em', color: '#C9A84C', opacity: 0.7 }}>01</span>
            <span style={{ fontFamily: "'Cinzel',serif", fontSize: '0.8rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#F5EDD6' }}>Select Your Service</span>
          </div>

          {/* Category filter */}
          <div style={{ display: 'flex', border: '1px solid rgba(201,168,76,0.15)', overflow: 'hidden', marginBottom: 20 }}>
            {CATS.map((cat, i) => (
              <button key={cat} onClick={() => setCatFilter(cat)}
                style={{ fontFamily: "'Cinzel',serif", fontSize: '0.58rem', letterSpacing: '0.18em', textTransform: 'uppercase', background: catFilter === cat ? 'rgba(201,168,76,0.08)' : 'transparent', border: 'none', color: catFilter === cat ? '#C9A84C' : '#D4C4A0', padding: '12px 0', flex: 1, cursor: 'pointer', borderRight: i < CATS.length - 1 ? '1px solid rgba(201,168,76,0.15)' : 'none', transition: 'all 0.25s' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {SERVICES.filter(s => catFilter === 'all' || s.cat === catFilter).map(svc => (
              <button key={svc.name} onClick={() => setSelected(svc)}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 20px', background: selected?.name === svc.name ? 'rgba(201,168,76,0.06)' : '#0e0e0e', border: `1px solid ${selected?.name === svc.name ? 'rgba(201,168,76,0.4)' : 'rgba(255,255,255,0.04)'}`, cursor: 'pointer', gap: 16, width: '100%', textAlign: 'left', transition: 'all 0.25s' }}>
                <div style={{ flex: 1 }}>
                  <p style={{ fontFamily: "'Cinzel',serif", fontSize: '0.75rem', letterSpacing: '0.08em', color: selected?.name === svc.name ? '#E8C96A' : '#F5EDD6', marginBottom: 4 }}>{svc.name}</p>
                  <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.9rem', color: '#D4C4A0', fontStyle: 'italic', opacity: 0.65 }}>{svc.meta}</p>
                </div>
                <span style={{ fontFamily: "'Cinzel',serif", fontSize: '1rem', fontWeight: 600, color: svc.cat === 'reading' ? '#C9A84C' : svc.cat === 'ritual' ? '#E05555' : svc.cat === 'healing' ? '#7B2FBE' : '#C9A84C', whiteSpace: 'nowrap' }}>
                  ${svc.price.toLocaleString()}
                </span>
                <div style={{ width: 18, height: 18, border: `1px solid ${selected?.name === svc.name ? '#C9A84C' : 'rgba(201,168,76,0.3)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', background: selected?.name === svc.name ? 'rgba(201,168,76,0.15)' : 'transparent', flexShrink: 0, fontSize: '0.7rem', color: '#C9A84C' }}>
        ...
        <div style={{ marginBottom: 48 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid rgba(201,168,76,0.12)' }}>
            <span style={{ fontFamily: "'Cinzel',serif", fontSize: '0.6rem', letterSpacing: '0.3em', color: '#C9A84C', opacity: 0.7 }}>02</span>
            <span style={{ fontFamily: "'Cinzel',serif", fontSize: '0.8rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#F5EDD6' }}>Choose Date &amp; Time</span>
          </div>
          <div style={{ marginBottom: 24 }}>
            <label style={labelStyle}>Date</label>
            <input type="date" min={minDate} value={date} onChange={e => setDate(e.target.value)} style={{ ...inputStyle, colorScheme: 'dark' }} />
          </div>
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
              {TIME_SLOTS.map(slot => (
                <button key={slot} onClick={() => setTime(slot)}
                  style={{ fontFamily: "'Cinzel',serif", fontSize: '0.6rem', letterSpacing: '0.1em', background: time === slot ? 'rgba(201,168,76,0.08)' : '#0e0e0e', border: `1px solid ${time === slot ? '#C9A84C' : 'rgba(255,255,255,0.06)'}`, color: time === slot ? '#C9A84C' : '#D4C4A0', padding: '12px 8px', cursor: 'pointer', textAlign: 'center', transition: 'all 0.2s' }}>
                  {slot}
                </button>
              ))}
            </div>
        ...
        <div style={{ marginBottom: 48 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid rgba(201,168,76,0.12)' }}>
            <span style={{ fontFamily: "'Cinzel',serif", fontSize: '0.6rem', letterSpacing: '0.3em', color: '#C9A84C', opacity: 0.7 }}>03</span>
            <span style={{ fontFamily: "'Cinzel',serif", fontSize: '0.8rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#F5EDD6' }}>Your Information</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              ...
              <textarea placeholder="Optional — a specific question, intention, or context for your session..." value={notes} onChange={e => setNotes(e.target.value)}
                style={{ ...inputStyle, resize: 'vertical', lineHeight: 1.6, minHeight: 80 }} rows={3} />
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ORDER SUMMARY */}
      <div style={{ position: 'sticky', top: 80, background: '#0e0e0e', border: '1px solid rgba(201,168,76,0.15)', padding: '36px 32px' }}>
        ...
        <button onClick={handleCheckout} disabled={!canCheckout || loading}
          style={{ width: '100%', fontFamily: "'Cinzel',serif", fontSize: '0.65rem', letterSpacing: '0.25em', textTransform: 'uppercase', color: '#080808', background: 'linear-gradient(135deg,#8B6914,#C9A84C,#E8C96A)', border: 'none', padding: 20, cursor: canCheckout ? 'pointer' : 'not-allowed', marginTop: 28, opacity: (!canCheckout || loading) ? 0.35 : 1, transition: 'all 0.3s' }}>
...
export default function BookPage() {
  return (
    <>
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(8,8,8,0.96)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(201,168,76,0.12)', padding: '16px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <Link href="/" style={{ fontFamily: "'Cinzel',serif", fontSize: '0.6rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: '#D4C4A0', textDecoration: 'none' }}>← Back to Site</Link>
      </nav>
