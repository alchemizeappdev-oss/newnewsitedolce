            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
              {TIME_SLOTS.map(slot => (
                <button key={slot} onClick={() => setTime(slot)}
                  style={{ fontFamily: "'Cinzel',serif", fontSize: '0.6rem', letterSpacing: '0.1em', background: time === slot ? 'rgba(201,168,76,0.08)' : '#0e0e0e', border: `1px solid ${time === slot ? '#C9A84C' : 'rgba(255,255,255,0.06)'}`, color: time === slot ? '#C9A84C' : '#D4C4A0', padding: '12px 8px', cursor: 'pointer', transition: 'all 0.2s' }}>
                  {slot}
                </button>
              ))}
            </div>
        ...
        <div style={{ marginBottom: 48 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div><label style={labelStyle}>First Name</label><input type="text" placeholder="First name" value={firstName} onChange={e => setFirstName(e.target.value)} style={inputStyle} /></div>
              <div><label style={labelStyle}>Last Name</label><input type="text" placeholder="Last name" value={lastName} onChange={e => setLastName(e.target.value)} style={inputStyle} /></div>
            </div>
            <div><label style={labelStyle}>Email Address</label><input type="email" placeholder="your@email.com" value={email} onChange={e => setEmail(e.target.value)} style={inputStyle} /></div>
            <div><label style={labelStyle}>Phone Number</label><input type="tel" placeholder="(775) 000-0000" value={phone} onChange={e => setPhone(e.target.value)} style={inputStyle} /></div>
            <div>
