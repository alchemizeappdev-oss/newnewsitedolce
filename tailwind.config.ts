...
export default function ServicesPage() {
  ...
  const tabBase: React.CSSProperties = {
    ...
    padding: '15px 0', flex: 1, cursor: 'pointer', transition: 'all 0.25s',
  ...
  return (
    <>
      <nav style={navStyle}>
        <Link href="/" style={{ fontFamily: "'Cinzel Decorative', serif", fontSize: '1.1rem', fontWeight: 700, letterSpacing: '0.12em', textDecoration: 'none' }}
          className="gold-text">Dolce Vida</Link>
        ...
        <Link href="/book" style={{ fontFamily: "'Cinzel', serif", fontSize: '0.6rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: '#080808', background: 'linear-gradient(135deg,#8B6914,#C9A84C,#E8C96A)', border: 'none', padding: '10px 22px', textDecoration: 'none', display: 'inline-block' }}>
          Book Now
        </Link>
      </nav>

      {/* Page header */}
      <div style={{ padding: '100px 20px 60px', textAlign: 'center', position: 'relative', background: '#080808' }}>
        ...
        <div style={{ display: 'flex', border: '1px solid rgba(201,168,76,0.2)', overflow: 'hidden' }}>
          {(['all', 'reading', 'ritual', 'healing'] as const).map((cat, i, arr) => (
            <button key={cat} onClick={() => setFilter(cat)}
              style={{ ...tabBase, borderRight: i < arr.length - 1 ? tabBase.borderRight : 'none', color: filter === cat ? '#C9A84C' : '#D4C4A0', background: filter === cat ? 'rgba(201,168,76,0.08)' : 'transparent' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px 100px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(320px,1fr))', gap: 2 }}>
          {visible.map(svc => (
            <div key={svc.name} style={{ background: '#0e0e0e', padding: 32, border: '1px solid rgba(255,255,255,0.04)', transition: 'all 0.3s', cursor: 'default' }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 20, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <button onClick={() => router.push(`/book?service=${encodeURIComponent(svc.name)}&price=${svc.price}`)}
                  style={{ fontFamily: "'Cinzel', serif", fontSize: '0.55rem', letterSpacing: '0.25em', textTransform: 'uppercase', color: '#C9A84C', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, transition: 'gap 0.2s' }}>
                  Book This →
                </button>
              </div>
            </div>
